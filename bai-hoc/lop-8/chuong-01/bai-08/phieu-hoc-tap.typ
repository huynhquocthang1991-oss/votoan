// ══════════════════════════════════════════════════════════════════
//  PHIẾU HỌC TẬP — Bài tập cuối chương I (Toán 8)
  ...
  header: context {
    if counter(page).get().first() > 1 {
      ...
      grid(columns: (1fr, auto),
        [Bài tập cuối chương I — Phiếu học tập], [Toán 8])
      v(-1mm)
  ...
  grid(columns: (1fr, auto), align: (left + horizon, right + horizon),
    {
      ...
      v(1.4mm)
      text(fill: primaryd, font: display, weight: 800, size: 24pt)[Bài tập cuối chương I]
      v(1.2mm)
      text(fill: muted, size: 11.5pt)[Toán 8 · Chương I — Đa thức]
    },
    text(font: display, weight: 800, size: 42pt, fill: primary.transparentize(72%))[08]))
...
#sub("Ba nhóm việc phải nhớ của chương I")
#lythuyet(
  ([*Đơn thức · Đa thức:* (thu gọn, hệ số, phần biến, bậc thế nào?)], 3),
  ([*Cộng · Trừ · Nhân:* (cách bỏ ngoặc, nhân đa thức?)], 3),
  ([*Chia cho đơn thức:* (thực hiện phép chia thế nào?)], 3))
...
#mcq("Bài 1.39", [Đơn thức $-2^3 x^2 y z^3$ có],
  [hệ số $-2$, bậc 8.], [hệ số $-2^3$, bậc 5.],
  [hệ số $-1$, bậc 9.], [hệ số $-2^3$, bậc 6.])

#mcq("Bài 1.40", [Gọi $T$ là tổng, $H$ là hiệu của $3x^2 y - 2x y^2 + x y$ và $-2x^2 y + 3x y^2 + 1$. Chọn khẳng định đúng.],
  [$T = x^2 y - x y^2 + x y + 1$, $H = 5x^2 y - 5x y^2 + x y - 1$.],
  [$T = x^2 y + x y^2 + x y + 1$, $H = 5x^2 y - 5x y^2 + x y - 1$.],
  [$T = x^2 y + x y^2 + x y + 1$, $H = 5x^2 y - 5x y^2 - x y - 1$.],
  [$T = x^2 y + x y^2 + x y - 1$, $H = 5x^2 y + 5x y^2 + x y - 1$.])

#mcq("Bài 1.41", [Tích của $6x^2 y z$ và $-2y^2 z^2$ là],
  [$4x^2 y^3 z^3$.], [$-12x^2 y^3 z^3$.],
  [$-12x^3 y^3 z^3$.], [$4x^3 y^3 z^3$.])

#mcq("Bài 1.42", [Chia $8x^3 y^2 - 6x^2 y^3$ cho $-2x y$, ta được],
  [$-4x^2 y + 3x y^2$.], [$-4x y^2 + 3x^2 y$.],
  [$-10x^2 y + 4x y^2$.], [$4x^2 y - 3x y^2$.])
...
#probm(c-bt, "Bài 1.43", [Một đa thức hai biến bậc hai thu gọn có thể có nhiều nhất:],
  ("a)", [bao nhiêu hạng tử bậc hai? (Cho ví dụ)], 3),
  ("b)", [bao nhiêu hạng tử bậc nhất? (Cho ví dụ)], 3),
  ("c)", [bao nhiêu hạng tử khác 0? (Cho ví dụ)], 3))

#probm(c-bt, "Bài 1.44", [Cho $3x^3(x^5 - y^5) + y^5(3x^3 - y^3)$.],
  ("a)", [Rút gọn biểu thức.], 5),
  ("b)", [Tính giá trị nếu $y^4 = x^4 sqrt(3)$.], 4))

#prob(c-bt, "Bài 1.45", [Rút gọn $1/4(2x^2 + y)(x - 2y^2) + 1/4(2x^2 - y)(x + 2y^2)$.], 7)

#v(1.4mm)
#block(breakable: false, {
  tag(c-bt, "Bài 1.46"); h(2mm); [Từ miếng bìa hình chữ nhật dài $y$ cm, rộng $z$ cm, cắt bốn hình vuông cạnh $x$ cm ở bốn góc rồi gấp thành hộp không nắp (Hình 1.3). Tìm đa thức biểu thị thể tích hộp và xác định bậc.]
  v(2mm)
  align(center, image("hinh-1-3.svg", width: 85%))
})
#dots(6)

#prob(c-bt, "Bài 1.47", [Biết $D$ là đơn thức và $-2x^3 y^4 : D = x y^2$. Tìm thương $(10x^5 y^2 - 6x^3 y^4 + 8x^2 y^5) : D$.], 6)

#prob(c-bt, "Bài 1.48", [Thực hiện $[8x^3(2x - 5)^2 - 6x^2(2x - 5)^3 + 10x(2x - 5)^2] : 2x(2x - 5)^2$.\ Hướng dẫn: Đặt $y = 2x - 5$.], 8)
