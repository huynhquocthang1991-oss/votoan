// ══════════════════════════════════════════════════════════════════
//  PHIẾU HỌC TẬP — Luyện tập chung (tr.45–46) (Toán 8)
  ...
  header: context {
    if counter(page).get().first() > 1 {
      ...
      grid(columns: (1fr, auto),
        [Luyện tập chung (tr.45–46) — Phiếu học tập], [Toán 8])
      v(-1mm)
  ...
  grid(columns: (1fr, auto), align: (left + horizon, right + horizon),
    {
      ...
      v(1.4mm)
      text(fill: primaryd, font: display, weight: 800, size: 24pt)[Luyện tập chung (tr.45–46)]
      v(1.2mm)
      text(fill: muted, size: 11.5pt)[Toán 8 · Chương II — Hằng đẳng thức đáng nhớ và ứng dụng]
    },
    text(font: display, weight: 800, size: 42pt, fill: primary.transparentize(72%))[14]))
...
#sec("1", "Định hướng lựa chọn phương pháp")
#sub("Em tự hoàn thiện sơ đồ")
#lythuyet(
  ([*Bước 1:* Khi nào phải ưu tiên đặt nhân tử chung?], 4),
  ([*Bước 2:* Dấu hiệu nhận ra hằng đẳng thức.], 6),
  ([*Bước 3:* Cách nhóm các hạng tử và kiểm tra phân tích triệt để.], 6))


#sec("2", "Phối hợp phương pháp")
#probm(c-lt, "Luyện tập 1", [Phân tích thành nhân tử.],
  ("a)", [$x^2 - x + 2y - 4y^2$], 8),
  ("b)", [$4x^2 - 4x + 1 - 9y^2$], 7),
  ("c)", [$x y + z^2 + x z + y z$], 7))
#prob(c-lt, "Luyện tập 2",
  [Phân tích $x^2 - 4x y + 4y^2 + x z - 2y z$ thành nhân tử.], 8)


#sec("3", "Nhận dạng cấu trúc")
#probm(c-vdung, "Vận dụng 1", [Phân tích thành nhân tử.],
  ("a)", [$x^3 + y^3 + x + y$], 8),
  ("b)", [$x^3 - y^3 + x - y$], 8),
  ("c)", [$(x - y)^3 + (x + y)^3$], 10))
#prob(c-vdung, "Vận dụng 2",
  [Phân tích $8x^3 - 27y^3 - 27y^2 - 9y - 1$ bằng cách nhận ra một lập phương hoàn chỉnh.], 10)


#sec("4", "Củng cố")
#probm(c-bt, "Bài tổng hợp", [Cho $P = x^3 - 3x^2 y + 3x y^2 - y^3 + y^2 - x^2$.],
  ("a)", [Nhận ra hằng đẳng thức trong bốn hạng tử đầu.], 4),
  ("b)", [Phân tích $P$ thành nhân tử.], 10),
  ("c)", [Kiểm tra kết quả bằng phép nhân.], 6))
#prob(c-bt, "Mở rộng Bézout",
  [Kiểm tra $x = 2$ là nghiệm của $2x^3 + x^2 - 3x - 14$, rồi tìm đa thức thương khi chia cho $x - 2$.], 10)
