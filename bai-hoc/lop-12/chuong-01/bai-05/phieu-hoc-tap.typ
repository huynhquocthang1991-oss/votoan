...
// ── Nội dung riêng của Bài 5. Ứng dụng đạo hàm để giải quyết vấn đề thực tiễn ──
#let TEN = [Bài 5. Ứng dụng đạo hàm để giải quyết vấn đề thực tiễn]
...
#dau-phieu(TEN, "05", [Toán 12 · Chương I — Ứng dụng đạo hàm])

#sec("1", "Tốc độ thay đổi của một đại lượng")

#lythuyet(
  ([Viết công thức tốc độ thay đổi *trung bình* và tốc độ thay đổi *tức thời* của $y$ theo $x$.], 5),
  ([Trong vật lí: $s(t)$ là hàm vị trí thì $s'(t)$ là …, $s''(t)$ là …], 3),
  ([Trong kinh tế: $C(x)$ là hàm chi phí thì $C'(x)$ gọi là gì và có ý nghĩa gì?], 3))

#prob(c-vd, "Ví dụ 1",
  [Độ cao của một vật là $h(t) = 2 + dc("24,5") t - dc("4,9") t^2$ (mét). Tìm vận tốc sau $2$ giây; khi nào vật đạt độ cao lớn nhất; khi nào vật chạm đất và vận tốc lúc đó?], 13)

#prob(c-vd, "Ví dụ 2",
  [Chi phí sản xuất $x$ đơn vị hàng hoá là $C(x) = 30 000 + 300 x - dc("2,5") x^2 + dc("0,125") x^3$ (nghìn đồng). Tìm hàm chi phí biên và tính $C'(200)$, rồi so sánh với chi phí sản xuất đơn vị thứ $201$.], 12)

#prob(c-lt, "Luyện tập",
  [Huyết áp tâm thu là $P(t) = (25 t^2 + 125)/(t^2 + 1)$ (mmHg), $0 <= t <= 10$ giây. Tính tốc độ thay đổi của huyết áp sau $5$ giây.], 9)

#sec("2", "Bài toán tối ưu hoá")

#lythuyet(
  ([Bước 1: xác định đại lượng nào? Biểu diễn nó qua cái gì?], 3),
  ([Bước 2: chọn biến ra sao? Cần tìm thêm điều gì của hàm số?], 3),
  ([Bước 3: làm gì rồi kết luận thế nào?], 3))

#prob(c-vd, "Ví dụ 3",
  [Một nhà sản xuất cần làm những hộp hình trụ thể tích $1$ lít. Tìm bán kính đáy và chiều cao để chi phí vật liệu nhỏ nhất (làm tròn đến chữ số thập phân thứ hai).], 15)

#prob(c-vdung, "Vận dụng",
  [Sân vận động sức chứa $55 000$ khán giả. Với giá vé $100$ nghìn đồng có $27 000$ khán giả; cứ giảm giá $10$ nghìn đồng thì thêm khoảng $3 000$ khán giả. Nên đặt giá vé bao nhiêu để doanh thu lớn nhất?], 14)

#prob(c-vd, "Ví dụ 4",
  [Lợi nhuận khi bán $x$ chiếc máy xay sinh tố mỗi tháng là $P(x) = -dc("0,3") x^3 + 36 x^2 + 1 800 x - 48 000$ (nghìn đồng). Phải bán ít nhất bao nhiêu chiếc để hoà vốn? Lợi nhuận lớn nhất là bao nhiêu?], 14)

#sec("3", "Bài tập · SGK tr.40 – 41")

#prob(c-bt, "Bài 1.26",
  [Toạ độ của một hạt trên trục thẳng đứng là $y = t^3 - 12 t + 3$ (mét), $t >= 0$. Tìm hàm vận tốc và gia tốc; khi nào hạt đi lên, khi nào đi xuống; tính quãng đường đi được trong $0 <= t <= 3$; khi nào hạt tăng tốc?], 16)

#prob(c-bt, "Bài 1.27",
  [Chi phí sản xuất $x$ đơn vị hàng hoá là $C(x) = 23 000 + 50 x - dc("0,5") x^2 + dc("0,00175") x^3$ (trăm nghìn đồng). Tìm hàm chi phí biên, tính $C'(100)$ và so sánh với chi phí sản xuất đơn vị thứ $101$.], 12)

#prob(c-bt, "Bài 1.28",
  [Một khu chung cư có $100$ căn hộ, cho thuê hết khi giá là $8$ triệu đồng một tháng; cứ tăng giá $100$ nghìn đồng thì thêm một căn bị bỏ trống. Nên đặt giá thuê bao nhiêu để doanh thu lớn nhất?], 13)

#prob(c-bt, "Bài 1.29",
  [Hàm cầu của một loại hàng hoá là $p = 354/(1 + dc("0,01") x)$ với $x >= 0$. Tìm $x$ như hàm số của $p$ và tập xác định; tính số sản phẩm bán được khi $p = 240$; nêu ý nghĩa của $lim_(p -> 0^+) x(p)$.], 13)

#sec("4", "Tự luyện thêm")

#prob(c-lt, "Bài 1",
  [Một vật chuyển động với quãng đường $s(t) = t^3 - 6 t^2 + 9 t$ (mét). Tìm các thời điểm vật đổi chiều chuyển động.], 7)

#prob(c-lt, "Bài 2",
  [Cần làm một chiếc hộp không nắp có đáy vuông với thể tích $32$ cm#super[3] sao cho diện tích vật liệu nhỏ nhất. Tìm các kích thước của hộp.], 9)

#prob(c-lt, "Bài 3",
  [Một mảnh đất hình chữ nhật nằm cạnh bờ sông chỉ cần rào ba mặt, tổng chiều dài hàng rào là $100$ m. Tìm diện tích lớn nhất của mảnh đất.], 8)
