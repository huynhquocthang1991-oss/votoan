...
// ── Nội dung riêng của Bài 3. Đường tiệm cận của đồ thị hàm số ──
#let TEN = [Bài 3. Đường tiệm cận của đồ thị hàm số]
...
#dau-phieu(TEN, "03", [Toán 12 · Chương I — Ứng dụng đạo hàm])

#sec("1", "Đường tiệm cận ngang")

#lythuyet(
  ([Đường thẳng $y = y_0$ là *tiệm cận ngang* của đồ thị $y = f(x)$ khi nào?], 4),
  ([Với hàm phân thức, muốn tính giới hạn tại vô cực thì chia tử và mẫu cho gì?], 3),
  ([Một đồ thị có thể có mấy tiệm cận ngang? Cho một ví dụ.], 3))

#prob(c-vd, "Ví dụ 1",
  [Tìm tiệm cận ngang của đồ thị hàm số $y = (3x - 2)/(x + 1)$.], 6)

#prob(c-vd, "Ví dụ 2",
  [Tìm các tiệm cận ngang của đồ thị hàm số $y = sqrt(x^2 + 1)/x$.], 9)

#prob(c-vdung, "Vận dụng",
  [Khối lượng còn lại của một chất phóng xạ sau $t$ ngày là $m(t) = 15 e^(-dc("0,012") t)$ (gam). Khối lượng đó thay đổi ra sao khi $t -> +oo$? Điều này thể hiện trên đồ thị như thế nào?], 8)

#sec("2", "Đường tiệm cận đứng")

#lythuyet(
  ([Đường thẳng $x = x_0$ là *tiệm cận đứng* của đồ thị $y = f(x)$ khi nào? Viết đủ bốn giới hạn.], 5),
  ([Ba bước tìm tiệm cận đứng của một hàm phân thức.], 4),
  ([Vì sao mẫu bằng $0$ *chưa chắc* đã cho tiệm cận đứng? Cho một ví dụ.], 4))

#prob(c-vd, "Ví dụ 3",
  [Tìm tiệm cận đứng của đồ thị hàm số $y = (3 - x)/(x + 2)$.], 8)

#prob(c-lt, "Luyện tập 1",
  [Tìm các tiệm cận ngang và tiệm cận đứng của đồ thị hàm số $y = (2x + 1)/(x - 4)$.], 10)

#prob(c-vdung, "Vận dụng",
  [Chi phí loại bỏ $p%$ một loài tảo độc khỏi hồ nước là $C(p) = (45 p)/(100 - p)$ (triệu đồng), $0 <= p < 100$. Tìm tiệm cận đứng của đồ thị và nêu ý nghĩa thực tiễn của nó.], 10)

#sec("3", "Đường tiệm cận xiên")

#lythuyet(
  ([Đường thẳng $y = a x + b$ với $a != 0$ là *tiệm cận xiên* của đồ thị $y = f(x)$ khi nào?], 4),
  ([Hai công thức tính $a$ và $b$.], 3),
  ([Với hàm phân thức, mẹo nào nhanh hơn việc tính hai giới hạn?], 3))

#prob(c-vd, "Ví dụ 4",
  [Tìm tiệm cận xiên của đồ thị hàm số $y = (x^2 - x + 2)/(x + 1)$ bằng cả hai cách.], 12)

#prob(c-lt, "Luyện tập 2",
  [Tìm các tiệm cận đứng và tiệm cận xiên của đồ thị hàm số $y = (x^2 - 4x + 2)/(1 - x)$.], 12)

#sec("4", "Bài tập · SGK tr.25")

#prob(c-bt, "Bài 1.17",
  [Đường thẳng $x = 1$ có phải là tiệm cận đứng của đồ thị hàm số $y = (x^2 + 2x - 3)/(x - 1)$ không? Vì sao?], 9)

#probm(c-bt, "Bài 1.18", [Tìm các tiệm cận của đồ thị các hàm số sau:],
  ("a)", [$y = (3 - x)/(2x + 1)$;], 9),
  ("b)", [$y = (2x^2 + x - 1)/(x + 2)$.], 11))

#prob(c-bt, "Bài 1.19",
  [Chi phí sản xuất $x$ sản phẩm là $C(x) = 2x + 50$ (triệu đồng). Chứng tỏ rằng chi phí trung bình $f(x) = C(x)/x$ giảm và có giới hạn bằng $2$ khi $x -> +oo$. Tính chất này nói lên điều gì?], 12)

#prob(c-bt, "Bài 1.20",
  [Một mảnh vườn hình chữ nhật có diện tích $144$ m#super[2], một cạnh dài $x$ (m). Viết biểu thức chu vi $P(x)$ rồi tìm các tiệm cận của đồ thị hàm số đó.], 12)

#sec("5", "Tự luyện thêm")

#prob(c-lt, "Bài 1",
  [Tìm các tiệm cận của đồ thị hàm số $y = (x^2 - 1)/(x^2 - 4)$.], 12)

#prob(c-lt, "Bài 2",
  [Tìm $m$ để đồ thị hàm số $y = (m x + 3)/(x - 2)$ có tiệm cận ngang $y = 4$.], 9)

#prob(c-lt, "Bài 3",
  [Tìm $m$ để đồ thị hàm số $y = (x^2 - 3x + m)/(x - 2)$ không có tiệm cận đứng.], 11)

#prob(c-lt, "Bài 4",
  [Tìm các tiệm cận của đồ thị hàm số $y = (2x^2 + 3x - 2)/(x - 1)$.], 9)

#prob(c-lt, "Bài 5",
  [Cho hàm số $y = sqrt(9 x^2 + 1)/(x + 2)$. Đồ thị hàm số có mấy tiệm cận ngang? Vì sao?], 7)
